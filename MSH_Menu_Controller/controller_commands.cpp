...
Controller::Controller(IDeviceService& d,
                       IModeService& m,
                       ISecurityService& s,
                       ISnapshotService& snap,
                       ILogger& log)
    : devices(d), modes(m), security(s), snapshots(snap), logger(log) {}
...
void Controller::run() {
    ...
    while (running) {
        ...
        switch (c) {
            ...
            case 2: modesMenu();     break;
            case 3: securityMenu();  break;
            case 4: snapshotsMenu(); break;
            case 0: running = false; logger.info("MSH exiting"); break;
            default: break;
        }
    }
...
public:
    ...
    void execute() override {
        ...
        if (list.empty()) std::cout << "(no devices)\n";
        else for (auto& s : list) std::cout << s << "\n";
...
public:
    ...
    void execute() override {
        for (auto& s : modes.listModes()) std::cout << s << "\n";
...
public:
    ...
    void execute() override {
        for (auto& a : sec.recentAlerts()) std::cout << a << "\n";
...
public:
    ...
    void execute() override {
        for (auto& s : snap.listSnapshots()) std::cout << s << "\n";
...
CommandPtr Controller::makeListDevicesCmd()   { return std::make_unique<ListDevicesCmd>(devices, logger); }
CommandPtr Controller::makeAddDeviceCmd()     { return std::make_unique<AddDeviceCmd>(devices, logger); }
CommandPtr Controller::makeRemoveDeviceCmd()  { return std::make_unique<RemoveDeviceCmd>(devices, logger); }
CommandPtr Controller::makeTurnOnCmd()        { return std::make_unique<TurnOnCmd>(devices, logger); }
CommandPtr Controller::makeTurnOffCmd()       { return std::make_unique<TurnOffCmd>(devices, logger); }
CommandPtr Controller::makeStatusCmd()        { return std::make_unique<StatusCmd>(devices, logger); }

CommandPtr Controller::makeListModesCmd()     { return std::make_unique<ListModesCmd>(modes, logger); }
CommandPtr Controller::makeSetModeCmd()       { return std::make_unique<SetModeCmd>(modes, logger); }
CommandPtr Controller::makeCurrentModeCmd()   { return std::make_unique<CurrentModeCmd>(modes, logger); }

CommandPtr Controller::makeArmCmd()           { return std::make_unique<ArmCmd>(security, logger); }
CommandPtr Controller::makeDisarmCmd()        { return std::make_unique<DisarmCmd>(security, logger); }
CommandPtr Controller::makeSecurityStatusCmd(){ return std::make_unique<SecurityStatusCmd>(security, logger); }
CommandPtr Controller::makeAlertsCmd()        { return std::make_unique<AlertsCmd>(security, logger); }

CommandPtr Controller::makeSaveSnapshotCmd()  { return std::make_unique<SaveSnapshotCmd>(snapshots, logger); }
CommandPtr Controller::makeListSnapshotsCmd() { return std::make_unique<ListSnapshotsCmd>(snapshots, logger); }
CommandPtr Controller::makeRestoreSnapshotCmd(){return std::make_unique<RestoreSnapshotCmd>(snapshots, logger); }
...
void Controller::devicesMenu() {
    ...
    while (true) {
        ...
        auto it = cmd.find(c);
        if (it != cmd.end()) it->second->execute();
...
void Controller::modesMenu() {
    ...
    while (true) {
        ...
        if (c == 1) listCmd->execute();
        else if (c == 2) setCmd->execute();
        else if (c == 3) curCmd->execute();
    }
}
...
void Controller::securityMenu() {
    ...
    while (true) {
        ...
        if (c == 1) armCmd->execute();
        else if (c == 2) disCmd->execute();
        else if (c == 3) statCmd->execute();
        else if (c == 4) alertCmd->execute();
    }
}
...
void Controller::snapshotsMenu() {
    ...
    while (true) {
        ...
        if (c == 1) saveCmd->execute();
        else if (c == 2) listCmd->execute();
        else if (c == 3) resCmd->execute();
    }
}
